function fmeaavg = speech_classifier(datasetFolder, labelsFile, samplingfreq, outputnet)
...
% run GMM on features
parfor l1=1:size(featuresTrain,1)
   l1
   gmdata = featuresTrain{l1,1}(:,:,1);
   if size(gmdata,2) <= 40
      padgm = zeros(40,41-size(gmdata,2));
      gmdata = [gmdata padgm];
   end
   gm = fitgmdist(gmdata',20,'Regularize',1e-5);
   featuresTrain{l1,1} = gm.mu';
   gmmAll{l1,1} = gm.mu';
end
...
for i = 1:numFolds
        ...
        gmmTrain = gmmAll(idxTrain);
        gmmVal = gmmAll(idxValidation);
        ...
        for l1=1:size(featuresTrain,1)
          l1
          featuresTrain{l1,1} = gmmTrain{l1,1};
        end 
        ...
        for l1=1:size(featuresValidation,1)
          l1
          featuresValidation{l1,1} = gmmVal{l1,1};
        end       
